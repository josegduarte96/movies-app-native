...
export class MovieDBRepository extends MovieRepository {
  ...
  async getNowPlaying({ page = 1 }: Options = {}): Promise<Paginated<Movie>> {
    try {
      const data = await this.fetcher.get<MovieDBMoviesResponse>('/movie/now_playing', {
        params: { page },
      });
  ...
  async getTopRated({ page = 1 }: Options = {}): Promise<Paginated<Movie>> {
    try {
      const data = await this.fetcher.get<MovieDBMoviesResponse>('/movie/top_rated', {
        params: { page },
      });
      return MovieMapper.fromResponseToPaginated(data);
    } catch (error) {
  ...
  async getPopular({ page = 1 }: Options = {}): Promise<Paginated<Movie>> {
    try {
      const data = await this.fetcher.get<MovieDBMoviesResponse>('/movie/popular', {
        params: { page },
      });
      return MovieMapper.fromResponseToPaginated(data);
    } catch (error) {
  ...
  async searchMovies(query: string, { page = 1 }: Options = {}): Promise<Paginated<Movie>> {
    if (query.trim().length === 0) {
      return { results: [], page: 1, totalPages: 0, totalResults: 0 };
    }

    try {
      const data = await this.fetcher.get<MovieDBMoviesResponse>('/search/movie', {
        params: { query, page },
      });
      return MovieMapper.fromResponseToPaginated(data);
    } catch (error) {
